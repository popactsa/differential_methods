...
void Solver::apply_boundary_conditions()
{
    ...
    int N_x = manager.parameters.N_x;
    ...
    for(int k = 0; k < 2; k++)
    {
        ...
        if(bound.name == "IN")
        {
            ...
            {
                fields[0][i] = 3./2.*fields[0][intern] - 1./2.*bound.rho;
                fields[1][i] = 3./2.*fields[1][intern] - 1./2.*bound.u*bound.rho;
                fields[2][i] = 3./2.*fields[2][intern] - 1./2.*bound.v*bound.rho;
                fields[3][i] = 3./2.*fields[3][intern] - 1./2.*bound.w*bound.rho;
                fields[4][i] = 3./2.*fields[4][intern] - 1./2.*bound.e*bound.rho;
            }
        ...
        else if(bound.name == "OUT")
        {
            ...
            for(int i = 0; i < fict; i ++)
            {
                fields[0][N_all-i-1] = -1./2.*fields[0][intern] + 3./2.*bound.rho;
                fields[1][N_all-i-1] = -1./2.*fields[1][intern] + 3./2.*bound.u*bound.rho;
                fields[2][N_all-i-1] = -1./2.*fields[2][intern] + 3./2.*bound.v*bound.rho;
                fields[3][N_all-i-1] = -1./2.*fields[3][intern] + 3./2.*bound.w*bound.rho;
                fields[4][N_all-i-1] = -1./2.*fields[4][intern] + 3./2.*bound.e*bound.rho;
            }
        ...
        else if(bound.name == "FLOW")
        {
            ...
            for(int i = 0; i < fict; i ++)
            {
                fields[0][N_all-i-1] = bound.rho;
                fields[1][N_all-i-1] = bound.rho*bound.u;
                fields[2][N_all-i-1] = bound.rho*bound.v;
                fields[3][N_all-i-1] = bound.rho*bound.w;
                fields[4][N_all-i-1] = bound.rho*bound.e;
            }
            // потоки надо считать тяжело
...
void Solver::solve_problem()
{
    ...
    for(int j = 0; j < 5; j++)
    {
        ...
        double alpha[2] {0};
        double omega[2] {0};
        double gamma[2] = {2.0/3, 1.0/3}; // стр. 112
        double beta[2] {0};
        for(int i = 1; i < N_x; i++)
        {
