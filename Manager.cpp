...
Manager::Manager(Parameters& param): parameters(param)
{
    ...
    dx = (param.x_end - param.x_start)/double(param.nx);
    N_all = param.nx + 2*fict;
    t_done = 0;
}
...
{
    xc = new double[parameters.nx+2*fict];
    for(int i = 0; i < parameters.nx+2*fict; i++)
    {
        xc[i] = dx*(1./2 + i - fict);
    }
    xb = new double[parameters.nx+2*fict+1];
    for(int i = 0; i < parameters.nx+2*fict+1; i++)
    {
        xb[i] = dx*(i - fict);
    }
}
...
void Manager::set_initial_conditions()
{
    ...
    for(int i = 0; i < N_all; i++)
    {
        ...
        u[i] = 1.0*0.1;
        e[i] = 8.31/(parameters.gamma - 1.0) + 1.0*(0.1*0.1)/2.;*/
        rho[i] = 1.0 + xc[i]*(1.0 - xc[i]);
        u[i] = rho[i]*u0;
        e[i] = p0/(parameters.gamma - 1.0) + rho[i]*(u0*u0)/2.;
        v[i] = 0;
        w[i] = 0;
    }
}
